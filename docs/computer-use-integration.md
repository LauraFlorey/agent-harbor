# Computer use & browser use in Agent Harbor

Decision doc, 2026-08-12. How bots in Agent Harbor get local computer use and
...
│     one TCC prompt, named Agent Harbor          │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is Agent Harbor's, users see ONE
   prompt named Agent Harbor, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `Agent Harbor.app/Contents/Resources/cua-driver`,
