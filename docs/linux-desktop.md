...
ARM64 are follow-ups in [upstream issue #29](https://github.com/milind-soni/OpenMausBot/issues/29).
...
- `release/Agent-Harbor-<version>-amd64.deb`
- `release/Agent-Harbor-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./release/Agent-Harbor-*-amd64.deb
```

Then open **Agent Harbor** from the GNOME application launcher. To remove it:

```sh
sudo apt remove agent-harbor
```

The portable AppImage does not install system files:

```sh
chmod +x release/Agent-Harbor-*-x86_64.AppImage
./release/Agent-Harbor-*-x86_64.AppImage
```

Application data remains local in the compatibility path `~/.openmausbot`. Electron browser data and window
state use the normal XDG configuration directory for Agent Harbor unless the environment overrides it.
...
./release/linux-unpacked/agent-harbor
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Agent Harbor
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/Agent-Harbor-*-x86_64.AppImage
```

Restart Agent Harbor after installing or signing in to a CLI.

## Xorg and Wayland

The baseline shell, chat, cloud computers, and connected apps work in both GNOME session types. Agent Harbor
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Agent Harbor. If it lives outside the
...
```sh
chmod +x Agent-Harbor-*-x86_64.AppImage
file Agent-Harbor-*-x86_64.AppImage
```
