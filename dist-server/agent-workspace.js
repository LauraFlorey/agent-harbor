...
function secureDirectory(path) {
    ...
    if (stat.isSymbolicLink())
        throw new Error(`refusing symbolic link for agent workspace: ${path}`);
    if (!stat.isDirectory())
        throw new Error(`agent workspace is not a directory: ${path}`);
    chmodSync(path, PRIVATE_DIR_MODE);
}
/** Create or reopen one stable, private workspace owned by a single bot. */
export function agentWorkspace(botId) {
    if (!SAFE_BOT_ID.test(botId))
        throw new Error("invalid bot id for agent workspace");
...
export function agentWorkingDirectory(bot) {
