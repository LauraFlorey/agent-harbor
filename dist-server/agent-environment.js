...
function setValue(target, key, value, platform) {
    if (platform === "win32") {
...
function allowedParentKey(key, testing) {
    const upper = key.toUpperCase();
    if (PARENT_KEYS.has(upper) || upper.startsWith("LC_"))
        return true;
...
export function buildAgentEnvironment(options = {}) {
    ...
    for (const [key, value] of Object.entries(parent)) {
        if (value !== undefined && allowedParentKey(key, testing))
