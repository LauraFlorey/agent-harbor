...
//   dweb_repo_status     → the Agent Harbor repo the daemon is tracking
...
const TOOLS = [
    {
        name: "dweb_status",
        description: "Ping the local dweb daemon and report its status: server id, hostname, platform, version, uptime, peer count, and the services it runs. Call this first to confirm the network is reachable and see what's available.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "dweb_repo_status",
        description: "Report the state of the Agent Harbor repository the dweb daemon is tracking: repo name, checked-out branch, last commit, file count, and checkout path.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "dweb_opencode_models",
        description: "List the models available on dweb's opencode integration. Each has an id you can pass to dweb_opencode_run. Returns a note if no models are available.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "dweb_opencode_run",
        description: "Run a model on dweb's opencode integration and wait for the result. Give a command for the model to process; optionally pick a model with the 'model' arg (defaults to dweb's choice). Can take several minutes — use this for long-running model work and report the output back verbatim.",
        inputSchema: {
            type: "object",
            properties: {
...
async function callTool(name, args) {
    ...
    if (name === "dweb_opencode_models") {
        ...
        if (!models.length)
            return { text: "No models available on dweb's opencode integration." };
    ...
    switch (method) {
        ...
        case "tools/call": {
            ...
            try {
                ...
                const args = typeof rawArgs === "object" && rawArgs !== null && !Array.isArray(rawArgs) ? rawArgs : {};
