...
Data currently remains in the compatibility path `~/.openmausbot/` (bots, transcripts, per-thread NDJSON
event logs, and non-secret configuration). Do not rename it without a tested migration.
